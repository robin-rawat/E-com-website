...
def add_to_cart(request, slug):
	...
	if order_qs.exists():
		...
		if order.items.filter(item__slug=item.slug).exists():
			...
			messages.info(request, "Item has been added in cart.")
...
def remove_from_cart(request, slug):
	...
	if order_qs.exists():
		...
		if order.items.filter(item__slug=item.slug).exists():
			...
			messages.info(request, "Item is not present in cart.")
			return redirect("core:product",slug=slug)

	else:
		messages.info(request, "You don't have any active order. Go ahead and press add to cart.")
		return redirect("core:product",slug=slug)
	return redirect("core:product",slug=slug)
